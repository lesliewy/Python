...
class DBQuery(object):
    ...
    def authorpoemcount(self, categoryname, authorname, poemname, poemurl):
        db = Mongo(database='poetry')
        db.poem.find({"category.name" : categoryname, "author.name" : authorname, "poem.name" : poemname, "poem.url" : poemurl}).count()

    # 查询某位诗人的诗词列表.
    @classmethod
    def authorpoems(self, categoryname, authorname):
        db = Mongo(database='poetry')
        db.poem.find({"category.name" : categoryname, "author.name" : author.name}, {"poems"})